...
/*
 ...
 The Brute Force approach was used where there are n * n threads
 for a graph with n vertices.
 Each thread checks for a single vertex in the 2D matrix.
 */
...
struct arg_struct //container
{
    int row; // The row number in the matrix.
    int column; // The column number in the matrix.
};

/*
 This method checks if a vertex assigned to
 a single thread has been set to 1.
 */
...
/*
This method creates n * n number of threads
for a given graph of n vertices.
Each thread will only check for 1 vertex in the graph.
*/
void openThreads()
{
    ...
    for(row= 0; row< size; row++) //call a check(args) thread for each position in the matrix
    {
        for(column = 0; column < size; column++)
        {
            ...
            args->row= row;
            args->column = column;
            error = pthread_create(&tid[j], NULL, &check, args);
            if (error != 0)
            {
...
int main() {
    ...
    //--------------------------------------------------------------------------
    // The 4 vertices Graph with 4 edges per node
